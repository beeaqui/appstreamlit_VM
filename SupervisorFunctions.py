...
def create_trace_wip(df, name, color):
    if df is not None and not df.empty:
        return go.Scatter(
            ...
            hoverinfo='text',
            line_shape='hv',
            hovertext=[f'Time: {time}<br>Cumulative Orders: {orders}'
...
def plot_generated_orders(width_plot):
    ...
    plotly_df = pd.DataFrame({'Time': [], 'Customer orders': []})
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            tickfont=dict(color='black'), title=dict(text='Time (seconds)', font_color='black'),
            showline=False,
            showgrid=False,
            showticklabels=True),
        yaxis=dict(
            tickfont=dict(color='black'), title=dict(text='Customer orders', font_color='black'),
            showline=False,
            showgrid=True,
            showticklabels=True),
...
def leadtime_plot(width_plot):
    ...
    fig = px.bar(df, x='Order Number', y='Lead Time', orientation='v',
                 ...
                 labels=({'Lead Time': 'Lead time (seconds)'}, {'Order Number': 'Order number'}))
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            ...
            showticklabels=True),
        yaxis=dict(
            tickfont=dict(color='black'), title=dict(text='Lead time (seconds)', font_color='black'),
            showline=False,
            showgrid=True,
            showticklabels=True),
        showlegend=False,
...
def solution_trajectory(coefficients, x_coefficients, y_coefficients, objective_coefficients,
    ...
    standard_coordinates, sensor_kit_coordinates = trajectory_plot(connect_mongodb())
